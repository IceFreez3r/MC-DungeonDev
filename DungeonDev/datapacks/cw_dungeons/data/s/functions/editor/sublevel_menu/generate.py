...
for i in range(max_sublevel):
    for j in range(max_sublevel):
        ...
        with open(filename, "w") as f:
            ...
            f.write('function s:storage/store/level/start\n')
            f.write("function s:editor/sublevel_menu/start\n")
...
def menu(sublevels):
    with open(f"{sublevels}.mcfunction", "w") as f:
        ...
        for i in range(sublevels - 1):
            f.write(f'{{"text": "  {i + 1}     "}}, ')
        f.write(f'{{"text": "-", "color": "red", "clickEvent": {{"action": "run_command", "value": "/function s:editor/sublevel_menu/decrease"}}}}')
        f.write(f', {{"text": " {sublevels} "}}')
        f.write(f', {{"text": "+", "color": "green", "clickEvent": {{"action": "run_command", "value": "/function s:editor/sublevel_menu/increase"}}}}]\n')
        # separator line
        ...
        for i in range(sublevels):
            ...
            for j in range(sublevels):
                f.write(f', {{"text": "-", "color": "red", "clickEvent": {{"action": "run_command", "value": "/function s:editor/sublevel_menu/decrease/{i}/{j}"}}}}')
                f.write(', {"text": " "}')
                f.write(f', {{"score": {{"objective": "dg.sublevel", "name": ".sublevel_{i}_{j}"}}}}')
                f.write(', {"text": " "}')
                f.write(f', {{"text": "+", "color": "green", "clickEvent": {{"action": "run_command", "value": "/function s:editor/sublevel_menu/increase/{i}/{j}"}}}}')
                f.write(', {"text": " | "}')
            f.write(']\n')

...
execute unless score .ongoing_run dg.build matches 1 if entity @e[type=marker,tag=dg.inactive] run tellraw @p[gamemode=creative] {"text": "Click to end the run", "color": "red", "clickEvent": {"action": "run_command", "value": "/function dg:build/stop"}}
